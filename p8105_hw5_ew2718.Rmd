...
*I'm going to combine the two tibbles I created earlier into one data frame.  Then, I'm going to write a function to do `prop.test`.  After, I'll apply the function to the data frame using mapping.* 

```{r}
homicide_props = left_join(unsolved_homicide, total_homicide, by = "city_state") %>%
  list(c("city_state")) %>% 
  list(c("unsolved")) %>%
  list(c("all_homicide")) %>%
  purrr::map2("unsolved","all_homicide", prop.test) %>%
...
for (i in 1:5000) {
  my_sample = rnorm(n = 30, mu = 0, sigma = 5)
  t_test = prop.test(my_sample, mu = 0, conf.level = 0.95)
}

sim_results = bind_rows(output_0)
```
