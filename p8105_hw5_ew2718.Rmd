...
for (i in 1:5000) {
  sim_results_mu0[[i]] = bind_rows(output, sim_function(mu = 0))
}

t_test_mu0 = sim_results_mu0 %>%
  as.numeric(sim_results_mu0$x) %>% 
  mutate(
    t_test_res = map(.x = x, ~t.test(.x) %>% 
                       broom::tidy()))
```
...
for (i in 1:5000) {
  sim_results_mu1[[i]] = bind_rows(output, sim_function(mu = 1))
}

## mu = 2
for (i in 1:5000) {
  sim_results_mu2[[i]] = bind_rows(output, sim_function(mu = 2))
}


## mu = 3
for (i in 1:5000) {
  sim_results_mu3[[i]] = bind_rows(output, sim_function(mu = 3))
}


## mu = 4
for (i in 1:5000) {
  sim_results_mu4[[i]] = bind_rows(output, sim_function(mu = 4))
}

## mu = 5
for (i in 1:5000) {
  sim_results_mu5[[i]] = bind_rows(output, sim_function(mu = 5))
}

## mu = 6
for (i in 1:5000) {
  sim_results_mu6[[i]] = bind_rows(output, sim_function(mu = 6))
}
